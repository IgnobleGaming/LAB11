...
TreeNode<T>* BinarySearchTree<T>::removeItem(TreeNode<T>* tNode, String* sk)
{
   //DO THIS
    ...
    if(comp == 0)
    {
        TreeNode<T>* toRemove = removeNode(tNode);
        return toRemove;
    }
    
    else if(comp < 0)
    {
        TreeNode<T>* item = removeItem(tNode->getLeft(),sk);
        tNode->setLeft(item);
    }
    
    else
    {
        TreeNode<T>* item = removeItem(tNode->getRight(),sk);
        tNode->setRight(item);
    }
...
TreeNode<T>* BinarySearchTree<T>::removeNode(TreeNode<T>* tNode)
{
   ...
   else 
   {
      //DO THIS
       ...
       tNode->setItem(temp);
       TreeNode<T>* toRemove = removeLeftMost(tNode->getRight());
       tNode->setRight(toRemove);
